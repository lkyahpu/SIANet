...
class matCapture:
    def __init__(self, path, gray=False):
        ...
        firstImage = scio.loadmat(self.images[self.num])['images']
    ...
    def read(self):
        if self.num >= len(self):
            return False, None
        # images = cv2.imread(self.images[self.num], cv2.IMREAD_GRAYSCALE if self.gray else cv2.IMREAD_COLOR)
        img = scio.loadmat(self.images[self.num])['images']
