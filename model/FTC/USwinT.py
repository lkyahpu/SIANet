...
class Attention_org(nn.Module):
    ...
    def forward(self, emb2,emb3,emb4, emb_all):
        # multi_head_Q1_list = []
        multi_head_Q2_list = []
        multi_head_Q3_list = []
        multi_head_Q4_list = []
...
class Block_ViT(nn.Module):
    ...
    def forward(self, emb2,emb3,emb4):
        embcat = []
        # org1 = emb1
        org2 = emb2
        org3 = emb3
        ...
        cx2,cx3,cx4, weights = self.channel_attn(cx2,cx3,cx4,emb_all)
...
class Encoder(nn.Module):
    ...
    def forward(self, emb2,emb3,emb4):
        ...
        for layer_block in self.layer:
            emb2,emb3,emb4, weights = layer_block(emb2,emb3,emb4)
            if self.vis:
        ...
        return emb2,emb3,emb4, attn_weights
...
class ChannelTransformer(nn.Module):
    ...
    def forward(self,en2,en3,en4):
        ...
        encoded2, encoded3, encoded4, attn_weights = self.encoder(emb2,emb3,emb4)  # (B, n_patch, hidden)
...
class USwinT(nn.Module):
    def __init__(self, config, n_channels=3, n_classes=1, img_size=256, vis=False):###224
        super().__init__()
        self.vis = vis
        ...
        self.sw = SwinTransformer(in_chans=3,
                            patch_size=4,
                            window_size=12,
                            embed_dim=128,
                            depths=(2, 2, 18, 2),
        ...
        self.reconstruct = Reconstruct(1024, 64, kernel_size=1, scale_factor=(32, 32))

    def forward(self, x):
        x = x.float()#(2,3,384,384)
        x0 = self.inc(x)#(2,64,384,384)
        x2 = self.down1(x0)#(2,128,192,192)
        x3 = self.down2(x2)#(2,256,96,96)
        x4 = self.down3(x3)#(2,512,48,48)
        x5 = self.down4(x4)#(2,512,24,24)


        #x2,x3,x4,att_weights = self.mtc(x2,x3,x4)
        x1 = self.sw(x)
