...
class _DCNv2(Function):
    ...
    ###############################################  修改的部分 ############################################################
    # 这里以下的修改，并不是必须的，仅仅是复现DCN的时候，输入改成是input和offset_mask，原始的做法是chunk分割为x, y, masks，
    ...
    # @staticmethod
    # def forward(ctx, input, offset, masks, weight, bias,
    #             stride, padding, dilation, deformable_groups):
    #     ctx.stride = _pair(stride)
    #     ctx.padding = _pair(padding)
    #     ctx.dilation = _pair(dilation)
...
class DCN(DCNv2):
    ...
    def forward(self, input):
        ...
        # o1, o2, masks = torch.chunk(out, 3, dim=1)
        # offset = torch.cat((o1, o2), dim=1)
        # masks = torch.sigmoid(masks)
        # return dcn_v2_conv(input, offset, masks,
...
class DCNPooling(DCNv2Pooling):
    ...
    def forward(self, input, rois):
        ...
        if not self.no_trans:
            ...
            # do pooling with offset and masks
            return dcn_v2_pooling(input, rois, offset,
                                  self.spatial_scale,
                                  self.pooled_size,
                                  self.output_dim,
