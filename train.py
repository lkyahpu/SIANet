...
parser.add_argument("--dataset_names", default=['PRCVdata'], type=list,
...
parser.add_argument("--save", default='./log3', type=str, help="Save path of checkpoints")
...
parser.add_argument("--threads", type=int, default=3, help="Number of threads for data loader to use")
...
def train(train_loader):
    ...
    for idx_epoch in range(epoch_state, opt.nEpochs):
        tbar = tqdm(train_loader, desc="Epoch {}".format(idx_epoch + 1))
        for idx_iter, (img, gt_mask) in enumerate(tbar):
            ...
            loss.backward()
            optimizer.step()
            tbar.set_description('training loss %.4f' % np.array(total_loss_epoch).mean())
        scheduler.step()
        if (idx_epoch + 1) % 10 == 0:
...
def test(save_pth):
    ...
    for idx_iter, (img, gt_mask, size, _) in enumerate(tbar):
        ...
        tbar.set_description('id %d' % idx_iter)
...
if __name__ == '__main__':
    for dataset_name in opt.dataset_names:
        ...
        for model_name in opt.model_names:
            ...
            train_set = TrainSetLoader(dataset_dir=opt.dataset_dir, dataset_name=opt.dataset_name,
